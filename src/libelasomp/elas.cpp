...
bool Elas::process(uint8_t *I1_, uint8_t *I2_, float *D1, float *D2,
                   const int32_t *dims)
{
  ...
  int32_t *disparity_grid_1 = (int32_t *)calloc(
      (param.disp_max + 2) * grid_height * grid_width, sizeof(int32_t));
  int32_t *disparity_grid_2 = (int32_t *)calloc(
      (param.disp_max + 2) * grid_height * grid_width, sizeof(int32_t));
...
#pragma omp sections
  {
#pragma omp section
    computeDisparity(p_support, tri_1, disparity_grid_1, grid_dims,
                     desc1.I_desc, desc2.I_desc, 0, D1);
#pragma omp section
    computeDisparity(p_support, tri_2, disparity_grid_2, grid_dims,
                     desc1.I_desc, desc2.I_desc, 1, D2);
  }
...
void Elas::removeInconsistentSupportPoints(int16_t *D_can, int32_t D_can_width,
                                           int32_t D_can_height)
{
    ...
    for (int32_t v_can = 0; v_can < D_can_height; v_can++)
    {
      ...
      if (d_can >= 0)
      {
             ...
             u_can_2 <= u_can + param.incon_window_size; u_can_2++)
        {
          ...
          {
            if (u_can_2 >= 0 && v_can_2 >= 0 && u_can_2 < D_can_width &&
                v_can_2 < D_can_height)
            {
              int16_t d_can_2 = *(D_can + getAddressOffsetImage(
                                              u_can_2, v_can_2, D_can_width));
...
void Elas::removeRedundantSupportPoints(int16_t *D_can, int32_t D_can_width,
                                        int32_t D_can_height,
                                        int32_t redun_max_dist,
                                        int32_t redun_threshold, bool vertical)
{
        ...
        for (int32_t i = 0; i < 2; i++)
        {
          ...
          for (int32_t j = 0; j < redun_max_dist; j++)
          {
            ...
            if (u_can_2 < 0 || v_can_2 < 0 || u_can_2 >= D_can_width ||
              ...
              break;
            d_can_2 =
                *(D_can + getAddressOffsetImage(u_can_2, v_can_2, D_can_width));
...
void Elas::addCornerSupportPoints(vector<support_pt> &p_support)
{
  ...
  // for right image
  p_border.push_back(
      support_pt(p_border[2].u + p_border[2].d, p_border[2].v, p_border[2].d));
  p_border.push_back(
      support_pt(p_border[3].u + p_border[3].d, p_border[3].v, p_border[3].d));
...
inline int16_t Elas::computeMatchingDisparity(const int32_t &u,
                                              const int32_t &v,
                                              uint8_t *I1_desc,
                                              uint8_t *I2_desc,
                                              const bool &right_image)
{
  ...
  if (u >= window_size + u_step && u <= width - window_size - 1 - u_step &&
      v >= window_size + v_step && v <= height - window_size - 1 - v_step)
  {
    ...
    // check if best and second best match are available and if matching ratio
    // is sufficient
...
vector<Elas::support_pt> Elas::computeSupportMatches(uint8_t *I1_desc,
                                                     uint8_t *I2_desc)
{
  ...
  for (int32_t v = 0; v < height; v += D_candidate_stepsize)
    D_can_height++;
  int16_t *D_can =
      (int16_t *)calloc(D_can_width * D_can_height, sizeof(int16_t));
...
#pragma omp parallel default(none) num_threads(2) private(                     \
    u_can, v_can, u, d, v, d2) shared(partial_p_support, lr_threshold, D_can,  \
                                      D_can_width, D_can_height,               \
                                      D_candidate_stepsize, I1_desc, I2_desc)
  {
    ...
    for (int32_t v_can = 1; v_can < D_can_height; v_can++)
      for (int32_t u_can = 1; u_can < D_can_width; u_can++)
        if (*(D_can + getAddressOffsetImage(u_can, v_can, D_can_width)) >= 0)
          partial_p_support[tid].push_back(support_pt(
              u_can * D_candidate_stepsize, v_can * D_candidate_stepsize,
              *(D_can + getAddressOffsetImage(u_can, v_can, D_can_width))));
  }
  p_support = partial_p_support[0];
  p_support.insert(p_support.end(), partial_p_support[1].begin(),
                   partial_p_support[1].end());
...
vector<Elas::triangle>
Elas::computeDelaunayTriangulation(vector<support_pt> p_support,
                                   int32_t right_image)
{
  ...
  {
    tri.push_back(triangle(out.trianglelist[k], out.trianglelist[k + 1],
...
void Elas::computeDisparityPlanes(vector<support_pt> p_support,
                                  vector<triangle> &tri, int32_t right_image)
{
...
void Elas::createGrid(vector<support_pt> p_support, int32_t *disparity_grid,
                      int32_t *grid_dims, bool right_image)
{
  ...
  int32_t *temp1 = (int32_t *)calloc(
      (param.disp_max + 1) * grid_height * grid_width, sizeof(int32_t));
  int32_t *temp2 = (int32_t *)calloc(
      (param.disp_max + 1) * grid_height * grid_width, sizeof(int32_t));
  ...
  for (int32_t i = 0; i < p_support.size(); i++)
  {
    ...
    for (int32_t d = d_min; d <= d_max; d++)
    {
      ...
      {
        int32_t addr =
            getAddressOffsetGrid(x, y, d, grid_width, param.disp_max + 1);
        *(temp1 + addr) = 1;
      }
    }
  }
  ...
  for (; br != end_input;
       tl++, tc++, tr++, cl++, cc++, cr++, bl++, bc++, br++, result++)
  ...
  for (int32_t x = 0; x < grid_width; x++)
  {
    for (int32_t y = 0; y < grid_height; y++)
    {
      ...
      for (int32_t d = 0; d <= param.disp_max; d++)
      {
        ...
        if (*(temp2 + getAddressOffsetGrid(x, y, d, grid_width,
                                           param.disp_max + 1)) > 0)
        {
          *(disparity_grid + getAddressOffsetGrid(x, y, curr_ind, grid_width,
                                                  param.disp_max + 2)) = d;
          curr_ind++;
        }
      }
      ...
      *(disparity_grid +
        getAddressOffsetGrid(x, y, 0, grid_width, param.disp_max + 2)) =
          curr_ind - 1;
    }
  }
...
inline void Elas::updatePosteriorMinimum(__m128i *I2_block_addr,
                                         const int32_t &d, const int32_t &w,
                                         const __m128i &xmm1, __m128i &xmm2,
                                         int32_t &val, int32_t &min_val,
                                         int32_t &min_d)
{
...
inline void Elas::updatePosteriorMinimum(__m128i *I2_block_addr,
                                         const int32_t &d, const __m128i &xmm1,
                                         __m128i &xmm2, int32_t &val,
...
inline void Elas::findMatch(int32_t &u, int32_t &v, float &plane_a,
                            float &plane_b, float &plane_c,
                            int32_t *disparity_grid, int32_t *grid_dims,
                            uint8_t *I1_desc, uint8_t *I2_desc, int32_t *P,
                            int32_t &plane_radius, bool &valid,
                            bool &right_image, float *D)
{
  ...
  int32_t d_plane =
      (int32_t)(plane_a * (float)u + plane_b * (float)v + plane_c);
  ...
  uint32_t grid_addr =
      getAddressOffsetGrid(grid_x, grid_y, 0, grid_dims[1], grid_dims[0]);
  ...
  if (!right_image)
  {
    ...
    for (d_curr = d_plane_min; d_curr <= d_plane_max; d_curr++)
    {
      ...
      if (u_warp < window_size || u_warp >= width - window_size)
        continue;
      updatePosteriorMinimum((__m128i *)(I2_line_addr + 16 * u_warp), d_curr,
                             valid ? *(P + abs(d_curr - d_plane)) : 0, xmm1,
                             xmm2, val, min_val, min_d);
    }
...
void Elas::computeDisparity(vector<support_pt> p_support, vector<triangle> tri,
                            int32_t *disparity_grid, int32_t *grid_dims,
                            uint8_t *I1_desc, uint8_t *I2_desc,
                            bool right_image, float *D)
{
  ...
  for (int32_t delta_d = 0; delta_d < disp_num; delta_d++)
    P[delta_d] = (int32_t)(
        (-log(param.gamma + exp(-delta_d * delta_d / two_sigma_squared)) +
         log(param.gamma)) /
        param.beta);
  int32_t plane_radius =
      (int32_t)max((float)ceil(param.sigma * param.sradius), (float)2.0);
...
// for all triangles do
#pragma omp parallel for num_threads(3) default(none) private(                 \
    i, plane_a, plane_b, plane_c, plane_d, c1, c2, c3)                         \
        shared(P, plane_radius, two_sigma_squared, disp_num, window_size,      \
               p_support, tri, disparity_grid, grid_dims, I1_desc, I2_desc,    \
               right_image, D)
      ...
      for (int32_t u = max((int32_t)A_u, 0); u < min((int32_t)B_u, width); u++)
      {
        if (!param.subsampling || u % 2 == 0)
        {
          ...
          for (int32_t v = min(v_1, v_2); v < max(v_1, v_2); v++)
            ...
            {
              findMatch(u, v, plane_a, plane_b, plane_c, disparity_grid,
                        grid_dims, I1_desc, I2_desc, P, plane_radius, valid,
                        right_image, D);
            }
        }
      }
        ...
        *(seg_list_v + 0) = v;
        ...
        while (seg_list_curr < seg_list_count)
        {
          ...
          for (int32_t i = 0; i < 4; i++)
          {
            ...
            if (u_neighbor[i] >= 0 && v_neighbor[i] >= 0 &&
                u_neighbor[i] < D_width && v_neighbor[i] < D_height)
            {

              // get neighbor pixel address
              addr_neighbor =
                  getAddressOffsetImage(u_neighbor[i], v_neighbor[i], D_width);
              ...
              if (*(D_done + addr_neighbor) == 0 && *(D + addr_neighbor) >= 0)
              {
                ...
                if (fabs(*(D + addr_curr) - *(D + addr_neighbor)) <=
                    param.speckle_sim_threshold)
                {
        ...
        if (seg_list_count < D_speckle_size)
        {
          ...
          {
            addr_curr = getAddressOffsetImage(*(seg_list_u + i),
                                              *(seg_list_v + i), D_width);
            *(D + addr_curr) = -10;
          }
        }
  ...
  for (int32_t v = 0; v < D_height; v++)
  {
    ...
    if (param.add_corners)
    {
      ...
      for (int32_t u = D_width - 1; u >= 0; u--)
      {
        ...
        if (*(D + addr) >= 0)
        {
          for (int32_t u2 = u; u2 <= min(u + D_ipol_gap_width, D_width - 1);
               u2++)
            *(D + getAddressOffsetImage(u2, v, D_width)) = *(D + addr);
          break;
        }
  ...
  for (int32_t u = window_size; u < D_width - window_size; u++)
  {
    for (int32_t v = window_size; v < D_height - window_size; v++)
    {
      if (*(D + getAddressOffsetImage(u, v, D_width)) >= 0)
      {
        ...
        *(D_temp + getAddressOffsetImage(u, v, D_width)) =
            *(vals + window_size);
      }
      else
      {
        *(D_temp + getAddressOffsetImage(u, v, D_width)) =
            *(D + getAddressOffsetImage(u, v, D_width));
      }
    }
  }
